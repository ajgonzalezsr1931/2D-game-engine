...
public class Gizmo extends Component {
	...
	public void editorUpdate(float dt) {
		...
		if (this.activeGameObject != null) {
			...
			//Todo: move this into it's own key editor biding component class
	...
	private boolean checkXHoverState() {
		...
		if (mousePos.x <= xAxisObject.transform.position.x  + (gizmoHeight / 2.0f) && //right x
				mousePos.x >= xAxisObject.transform.position.x - (gizmoWidth / 2.0f) && //left x 
				mousePos.y >= xAxisObject.transform.position.y - (gizmoHeight / 2.0f) && //top y
				mousePos.y <= xAxisObject.transform.position.y + (gizmoWidth / 2.0f)){ //bottom y
